...
function custom_nav_update($menu_id, $menu_item_db_id, $args)
{
    if (is_array($_REQUEST['menu-item-image'])) {
        $custom_value = sanitize_text_field($_REQUEST['menu-item-image'][$menu_item_db_id]);
...
{
    return 'Walker_Nav_Icon_Menu';
}

class Walker_Nav_Icon_Menu extends Walker_Nav_Menu
{
    ...
    <li id="menu-item-<?php echo $item_id; ?>" class="<?php echo implode(' ', $classes); ?>">
        ...
        <div class="menu-item-settings" id="menu-item-settings-<?php echo $item_id; ?>">
            ...
            ?>
            <p class="field-image-icon description description-wide">
                <label for="edit-menu-item-subtitle-<?php echo $item_id; ?>">
                    <?php _e('Image or Icon'); ?><br/>
                    <input type="hidden" id="edit-menu-item-subtitle-<?php echo $item_id; ?>"
                           class="widefat code edit-menu-item-image-icon"
                           name="menu-item-image[<?php echo $item_id; ?>]"
                           value="<?php echo esc_attr($item->image); ?>"/>
                    <input type="button" value="Choose Image or Icon"
                           class="ico-choose-btn-<?php echo $item_id; ?> button">
                    <?php $image = array(
                        0 => '#',
                        1 => 0,
                        2 => 0,
                    ...
                    <img src="<?php echo $image[0]; ?>" alt="" class="ico-preview"
                         style="max-width: 80px; display: block;">
                    <span class="remove-ico-<?php echo $item_id; ?>"
                          style="color:#A00; display: <?php echo $display; ?>;">Remove Image</span>
                </label>
            </p>
            <?php
        ...
        <script>
            jQuery(document).ready(function ($) {
                $('.ico-choose-btn-<?php echo $item_id; ?>').click(function (event) {
                    ...
                    var file_frame = wp.media.frames.file_frame = wp.media({
                        ...
                        library: {},
                        button: {text: 'Select one'},
                        multiple: false
                    });

                    // Pre select images
                    file_frame.on('open', function () {
                        var image = _t.parents('.field-image-icon').find('input[type="hidden"]').val();
                    ...
                    file_frame.on('select', function () {
                        ...
                        $.each(attachment, function (index, item) {
                            if (index == 0) {
                                _t.parents('.field-image-icon').find('.remove-ico').show();
                                _t.parents('.field-image-icon').find('img').attr('src', item.url);
                                _t.parents('.field-image-icon').find('input[type="hidden"]').val(item.id);
                            }
                            return false;
                        });
                    });
                ...
                $('.remove-ico-<?php echo $item_id; ?>').click(function (event) {
                    event.preventDefault();
                    $(this).hide();
                    $(this).parents('.field-image-icon').find('img').attr('src', '#');
                    $(this).parents('.field-image-icon').find('input[type="hidden"]').val('');
                });
            });
        </script>
        <!-- / JS Scripts -->
...
// Custom Main Menu
class Icon_Menu extends Walker_Nav_Menu
{
    function start_lvl(&$output, $depth = 0, $args = array())
    {
        $indent = str_repeat("\t", $depth);
        $output .= "\n$indent<ul class=\"sub-menu posa clearfix\">\n";
    }
    ...
    function start_el(&$output, $item, $depth = 0, $args = array(), $id = 0)
    {
        ...
        // Get image if exists
        $image_src = '';
        $item_image = get_post_meta($item->ID, '_menu_item_image', true);
        if ($item_image != '') {
            $image = wp_get_attachment_image_src($item_image, 'full');
            $image_src = $image[0];
        } else {
            if ($depth > 0)
                $image_src = get_template_directory_uri() . '/img/submenu-icon.png';
        }
        ...
        if ($depth <= 0) {
            ...
            $item_output .= '<span><img src="' . $image_src . '" alt=""></span>';
            $item_output .= '<span class="Menu__Text">' . apply_filters('the_title', $item->title, $item->ID) . '</span>';
            $item_output .= '</a>';
        } else {
            $item_output .= '<span class="posa"><img src="' . $image_src . '" alt=""></span>';
            $item_output .= '<a' . $attributes . '>';
